...
export const SearchInput: React.FC<Props> = ({ className }) => {
  ...
  React.useEffect(() => {
    ...
    Api.products.search(searchQuery).then((res: SearchResponse) => {
  ...
  // Загружаем при фокусе (пустой input)
  const handleFocus = () => {
    setFocused(true);
    if (!searchQuery.trim()) {
      Api.products.search('').then((res: SearchResponse) => {
        if (Array.isArray(res.products)) {
